...
public class TableGenerator extends AbstractConcreteGenerator {
  ...
  public TableGenerator(BaseTypeGenerator base, TypeDescriptor descriptor, String packageName) {
    ...
    for (FieldDescriptor fieldDescriptor : descriptor.getFields()) {
      ...
      FieldSpec field =
          FieldSpec.builder(fieldTypeName, fieldName).addModifiers(Modifier.PRIVATE).build();
      fields.add(field);
  ...
  protected void fillType() {
    fieldCount =
        FieldSpec.builder(int.class, "FIELD_COUNT")
    ...
    for (int i = 0; i < fields.size(); i++) {
      ...
      MethodSpec getter =
          MethodSpec.methodBuilder("get" + upperFirstChar(field.name))
  ...
  private void fillEmptyTypeBuilder() {
    ...
    MethodSpec.Builder constructorBufBuilder =
        constructorBufBuilder()
            .addStatement(
                "int size = $T.littleEndianBytes4ToInt(buf, 0)", base.classNameMoleculeUtils)
            .beginControlFlow("if (buf.length != size)")
            .addStatement(
                "throw new $T(size, buf.length, $T.class)", base.classNameMoleculeException, name)
            .endControlFlow()
            .addStatement("int[] offsets = $T.getOffsets(buf)", base.classNameMoleculeUtils)
            .beginControlFlow("if (offsets.length - 1 != $N)", fieldCount)
            .addStatement(
                "throw new $T(\"Raw data should have \" + $N + \" but find \" + (offsets.length -1) + \" offsets in header.\")",
  ...
  private void fillNonEmptyTypeBuilder() {
    ...
    for (int i = 0; i < fields.size(); i++) {
      ...
      if (field.type == TypeName.BYTE) {
        ...
        if (!isOption.get(i)) {
          ...
          constructorBufBuilder
              .addStatement(
                  "itemBuf = $T.copyOfRange(buf, offsets[$L], offsets[$L])", Arrays.class, i, i + 1)
    ...
    for (int i = 0; i < fields.size(); i++) {
      ...
      MethodSpec.Builder setterBuilder =
          MethodSpec.methodBuilder("set" + upperFirstChar(field.name))
              .addModifiers(Modifier.PUBLIC)
              .returns(builderName);
      ...
      if (!isOption.get(i)) {
        setterBuilder
            .addParameter(
                ParameterSpec.builder(field.type, field.name).addAnnotation(Nonnull.class).build())
            .addStatement("$T.requireNonNull($L)", Objects.class, field.name);
      } else {
        setterBuilder.addParameter(
            ParameterSpec.builder(field.type, field.name).addAnnotation(Nullable.class).build());
      }
      setterBuilder
    ...
    for (int i = 0; i < fields.size() - 1; i++) {
      ...
      if (field.type == TypeName.BYTE) {
        ...
        buildBuilder.addStatement(
            "offsets[$L] = offsets[$L] + ($L == null ? 0 : 1)", i + 1, i, field.name);
      } else {
        if (!isOption.get(i)) {
          buildBuilder.addStatement(
    ...
    for (int i = 0; i < fields.size(); i++) {
      ...
      if (field.type == TypeName.BYTE) {
        ...
        if (!isOption.get(i)) {
          ...
          buildBuilder.addStatement(
              "fieldsSize[$L] = ($L == null ? 0 : $L.getSize())", i, field.name, field.name);
        }
      }
    }
    ...
    for (int i = 0; i < fields.size(); i++) {
      ...
      if (field.type == TypeName.BYTE) {
        ...
        buildBuilder.addStatement(
            "fieldsBuf[$L] = ($L == null ? new byte[]{} : new byte[]{$L})",
            i,
            field.name,
            field.name);
      } else {
