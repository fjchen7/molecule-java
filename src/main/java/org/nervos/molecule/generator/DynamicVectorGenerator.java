...
public class DynamicVectorGenerator extends VectorGenerator {
  ...
  void fillTypeBuilderConstructor() {
    MethodSpec constructor =
        constructorBuilder().addStatement("items = new $T[0]", itemTypeName).build();
    ...
    constructorBufBuilder
        ...
        .addStatement(
            "byte[] itemBuf = $T.copyOfRange(buf, offsets[i], offsets[i + 1])", Arrays.class);

    if (isItemOption) {
      if (isItemBoxByte) {
        constructorBufBuilder.addStatement(
            "items[i] = (itemBuf.length == 0 ? null: buf[0])", itemTypeName);
      } else {
        constructorBufBuilder.addStatement(
            "items[i] = (itemBuf.length == 0 ? null: $T.builder(itemBuf).build())", itemTypeName);
      }
    } else {
      constructorBufBuilder.addStatement("items[i] = $T.builder(itemBuf).build()", itemTypeName);
    }
    constructorBufBuilder.endControlFlow();

    typeBuilderBuilder.addMethod(constructor).addMethod(constructorBufBuilder.build());
  }
  ...
  void fillTypeBuilderMethodBuild() {
    ...
    buildBuilder.addStatement("start += 4").endControlFlow();

    buildBuilder.beginControlFlow("for (int i = 0; i < items.length; i++)");
    ...
    if (isItemOption) {
      ...
      if (isItemBoxByte) {
        buildBuilder
            .addStatement(
                "$T.setBytes(new byte[]{items[i]}, buf, start);", base.classNameMoleculeUtils)
