...
public class BaseTypeGenerator extends AbstractGenerator {
  ...
  private TypeSpec generateBaseUnion() {
    TypeSpec union =
        TypeSpec.classBuilder("Union")
            .addModifiers(Modifier.PUBLIC, Modifier.ABSTRACT)
            .superclass(classNameMolecule)
            .addField(
                FieldSpec.builder(int.class, "typeId").addModifiers(Modifier.PROTECTED).build())
            .addField(
                FieldSpec.builder(Object.class, "item").addModifiers(Modifier.PROTECTED).build())
            .addMethod(
                MethodSpec.methodBuilder("getTypeId")
                    .addModifiers(Modifier.PUBLIC)
                    .returns(int.class)
                    .addStatement("return this.typeId")
                    .build())
            .addMethod(
                MethodSpec.methodBuilder("getItem")
                    .addModifiers(Modifier.PUBLIC)
  ...
  public TypeSpec generateMoleculeUtils() {
    ...
    MethodSpec getOffsets =
        MethodSpec.methodBuilder("getOffsets")
            ...
            .addStatement(
                "throw new $T(\"Byte length is less than 4.\")", this.classNameMoleculeException)
            .endControlFlow()
            .beginControlFlow("if (buf.length == 4)")
